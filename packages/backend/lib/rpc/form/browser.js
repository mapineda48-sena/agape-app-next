...
const { ArgsKey } = require("./integration.json");
...
function toFormData(args) {
  ...
  // Extract date entries from the arguments
...
function extractInstances(payload, instanceType, baseKey = "") {
  const instances = _.transform(
    ...
    (result, value, key) => {
      ...
      // If the value is a plain object or array, recursively process it
